...
void proximity(int id, std::vector<std::vector<float>> points,std::vector<int>& cluster, KdTree* tree, float distanceTol, std::vector<bool>& processed)
{
    processed[id] = true;
    cluster.push_back(id);
    for (int pt_id : tree->search(points[id], distanceTol))
    {
        if (!processed[pt_id])
            proximity(pt_id, points, cluster, tree, distanceTol, processed);
    }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int i=0; i<n; i++)
    {
	    if (!processed[i])
        {
            ...
            proximity(i, points, cluster, tree, distanceTol, processed);
...
int main ()
{
    ...
    auto time_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count();
    std::cout << "Kdtree insertion took " << time_elapsed << " nanoseconds.\n";
